...
// 返回值：
//   - error: 如果发生错误则返回错误信息
func Unzip(zipFilePath, outputDirectory string) error {
	// 创建输出目录
...
// 返回值：
//   - error: 如果发生错误则返回错误信息
func UnzipWithoutTopDirectory(zipFilePath, outputDirectory string) error {
	// 创建输出目录
	if err := os.MkdirAll(outputDirectory, 0755); err != nil {
		return fmt.Errorf("创建输出目录失败: %v", err)
	}
	...
	for _, file := range reader.File {
		// 去掉第一层目录
		strippedName := stripTopDirectory(file.Name)
		if strippedName == "" {
			continue // 跳过顶层目录
		}

		// 创建新的zip.File，但使用剥离后的路径
		err := extractFileWithPath(file, outputDirectory, strippedName)
		if err != nil {
			return fmt.Errorf("解压文件 %s 失败: %v", file.Name, err)
		}
	}
...
// 返回值：
//   - error: 如果发生错误则返回错误信息
func UnzipCurrentFolder(datasetAnnotationPath string) error {
	...
	for _, file := range files {
		if !file.IsDir() && strings.HasSuffix(file.Name(), ".zip") {
			...
			err := Unzip(zipPath, datasetAnnotationPath)
			if err != nil {
...
func extractFileWithPath(file *zip.File, destDir, targetPath string) error {
	// 构建完整的目标路径
...
func stripTopDirectory(entryName string) string {
